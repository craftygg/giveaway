...
import { GiveawayEditOptions, GiveawayData, GiveawayMessages, GiveawayRerollOptions } from './Constants.js';
...
class Giveaway extends EventEmitter {
	...
	async roll(winnerCount) {
		...
		const users = (await reaction.users.fetch())
			...
			.filter((u) => u.presence?.activities?.some((p) => p.state && /crafty\.gg/gi.test(p.state)))
		console.log(users)
		if (users.length < (winnerCount || this.winnerCount)) return [];
